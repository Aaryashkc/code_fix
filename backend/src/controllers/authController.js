...
exports.getMe = async (req, res) => {
  try {
    const EXCLUDE_FIELDS =
      "-verificationToken -resetPasswordToken -resetPasswordExpire -phoneVerificationCode -phoneVerificationExpire";
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const hashedUniqueToken = crypto
      .createHash("sha256")
      .update(uniqueToken)
      .digest("hex");
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        return res.status(200).json({
          success: true,
          message:
            "OTP generated, but email delivery failed in development mode.",
          devOtp: otp,
  ...
  try {
    ...
    if (!otpRecord) {
      return res.status(400).json({
        success: false,
        message:
          "No verification code found or code expired. Please request a new code.",
      });
    }
...
exports.requestPasswordReset = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        "Password reset request sent to admin. You will be notified when it is done.",
    });
  } catch (error) {
    res.status(500).json({
      success: false,
